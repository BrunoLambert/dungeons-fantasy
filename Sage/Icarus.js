const onUseItem = () => {
  try {
    const targetedNouliths = game.user.targets
      .toObject()
      .filter((token) => token.name === "Noulith");
    ...
    const createStyles = () => {
      const styles = {
        ...
        "#icarus-dialog .targets-selected h3": ["flex: 1"],
        "#icarus-dialog .targets-selected select": ["width: 100px"],
      };
    ...
    const options = Array.from({ length: targetedNouliths.length + 1 }).map(
      (_, index) => {
    ...
    const handleSelection = (html) => {
      const elements = html.find("select.deploy-select");
      const params = elements
        .toArray()
        .map((t) => ({ name: t.name, value: t.value }));

      const macro = game.macros.getName("Icarus Attach");
      if (macro) {
        macro.execute({ params });
      } else {
        ui.notifications.warn(
          "It wasn't possible to execute the Icarus Macro!"
        );
      }
    };
    ...
    let choiseDialog = new Dialog({
      ...
      content: `
                ...
                <div id="icarus-dialog" class="targets-container">
                    ...
                    <p>You selected ${
                      targetedNouliths.length
                    } Nouliths to deploy</p>
                ...
                <script>
                    ...
                    window.icarusInterval = setInterval(() => {
                        ...
                        newTargets.forEach(target => {
                            ...
                            elements += "<select class=" + "deploy-select" + " name=" + target.document.actor.id + ">${options.join(
                              " "
                            )}</select>"
                            elements += "</div>"
                        })
